...
\begin{equation}
  \mathcal{M}^{-1} r = \mathcal{F}^{-1}\!\left[
    \frac{\hat{r}(\kvec)}{4\pi^2 D_0|\kvec|^2 + \bar{\phi}_s/\eta}
  \right].
\end{equation}
...
\begin{equation}
  \hat{\bfu}^{n+1} = P_\text{sol}\!\left[
    \frac{\lambda\hat{\bfu}^n
          ...
          + \hat{\bff}}
         {\mu\,4\pi^2|\kvec|^2 + \lambda}
  \right],
\end{equation}
where $P_\text{sol}(\hat\bfv)_k = \hat\bfv_k - (\hat\bfv_k\cdot\hat{\kvec})\hat{\kvec}$
...
H.~Hasimoto.
\newblock Periodic solutions of one-dimensional and three-dimensional Navier-Stokes equations for the flow of viscous fluid through lattices of obstacles.
\newblock {\em Journal of Fluid Mechanics}, 5(2):317--328, 1959.
...
\begin{description}
  ...
  \item[Tetrahedral FEM bridge] \texttt{porescalemc/geometry/mesher.py}, \texttt{porescalemc/solvers/gmsh\_mesher.py}, and \texttt{porescalemc/solvers/tet.py}: optional Gmsh-based mesh generation supports oriented ellipsoids through affine transformations of the unit sphere. The current \texttt{TetDiffusionSolver} assembles a linear tetrahedral finite-element Laplace operator on the fluid mesh and reports directional Dirichlet diffusion estimates, mesh porosity, and element count. A fully periodic FEM cell problem remains future work.
